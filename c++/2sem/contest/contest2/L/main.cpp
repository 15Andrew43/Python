...
int main() {
    ...
    Segment segment(B, R);
    if (getDistance(T, segment) >= Rt) {
        std::cout << std::fixed << std::setprecision(10) << getDistance(B, R) << '\n';
    } else {
        long double l1 = sqrt(getDistance(B, T) * getDistance(B, T) - Rt * Rt);
        long double l2 = sqrt(getDistance(R, T) * getDistance(R, T) - Rt * Rt);
        Vector BT(B, T);
        Vector BR(B, R);
        long double sinTBR = fabs(BT.VectorProduct(BR)) / (BT.getLength() * BR.getLength());
        long double TBR = asin(sinTBR);
        Vector RT(R, T);
        Vector RB(R, B);
        long double sinTRB = fabs(RT.VectorProduct(RB)) / (RT.getLength() * RB.getLength());
        long double TRB = asin(sinTRB);

        long double BTP = atan(l1 / Rt);
        long double RTQ = atan(l2 / Rt);

        long double PTQ = PI - (TBR + TRB + BTP + RTQ);

        long double l3 = Rt * PTQ;

        long double L = l1 + l2 + l3;
